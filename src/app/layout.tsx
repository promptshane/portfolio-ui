...
const titleMap: { test: RegExp; title: string }[] = [
  { test: /^\/$/, title: "Home" },
  { test: /^\/analysis\b/, title: "Analysis" },
  { test: /^\/crypto\b/, title: "Crypto" },
  { test: /^\/dev\b/, title: "Developer" },
  { test: /^\/hedge\b/, title: "Hedge" },
  { test: /^\/login\b/, title: "Login" },
  { test: /^\/news\b/, title: "News" },
  { test: /^\/notes\b/, title: "Notes" },
  { test: /^\/options\b/, title: "Options" },
  { test: /^\/portfolio\b/, title: "Portfolio" },
  { test: /^\/profile\/leaderboard\b/, title: "Leaderboard" },
  { test: /^\/profile\b/, title: "Profile" },
  { test: /^\/settings\b/, title: "Settings" },
  { test: /^\/watchlist\b/, title: "Watchlist" },
];

export async function generateMetadata(): Promise<Metadata> {
  const hdrs = await headers();
  const path = hdrs.get("next-url") || "/";
  const found = titleMap.find(({ test }) => test.test(path));
  if (found) {
    return {
      title: { absolute: found.title },
